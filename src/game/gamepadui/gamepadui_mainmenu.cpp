...
void GamepadUIMainMenu::ApplySchemeSettings(vgui::IScheme* pScheme)
{
	...
	// added for a quick fix with emulsion logo
	const char* pOverWidth = pScheme->GetResourceString("Logo.ExtraWidth");
	if (pOverWidth && *pOverWidth)
		m_nExtraWidth = V_atoi(pOverWidth);

	const char* pOverHeight = pScheme->GetResourceString("Logo.ExtraHeight");
	if (pOverHeight && *pOverHeight)
		m_nExtraHeight = V_atoi(pOverHeight);

	// background movie stuff
	const char* pMovie = pScheme->GetResourceString("Background.Movie");
	if (pMovie && *pMovie) {
		GamepadUI::GetInstance().GetBaseUIPanel()->SetBackgroundMovie(pMovie);
		GamepadUI::GetInstance().GetBaseUIPanel()->SetBackgroundMovieEnabled(true);
	}
...
void GamepadUIMainMenu::PaintLogo()
{
	...
	{
		int nY1 = nLogoY;
		int nY2 = nY1 + nLogoH[0] + m_nExtraHeight;
		int nX1 = m_flLogoOffsetX;
		int nX2 = nX1 + (nLogoH[0] * 3) + m_nExtraWidth;
...
void GamepadUIMainMenu::UpdateButtonVisibility()
{
	for (CUtlVector<GamepadUIButton*>& buttons : m_Buttons)
	{
		...
		{
			GamepadUIButton* pButton = buttons[i];
